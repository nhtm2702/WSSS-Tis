...
class Stage1_TrainDataset(Dataset):
    ...
    def path_label(self):
        ...
        ## do os.walk nên phải ấn run stage_1 2 lần mới được -> cần fix
        # for root, dirname, filename in os.walk(self.data_path):
            # for f in filename:
            #     image_path = os.path.join(root, f)
            #     print(image_path)
...
class Stage2_Dataset(Dataset):
    def __init__(self, args, base_dir, split):
        ...
        self.args = args
        if self.split == "train":
            self.filenames = [os.path.splitext(file)[0] for file in os.listdir(self._cat_dir) if not file.startswith('.')]
